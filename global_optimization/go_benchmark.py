...
class Benchmark(object):
    ...
    def plot(self, xzero, xopt, xvals, xglob, set_title=False):
...
##        plt.rcParams['text.usetex'] = True
        ...
        name = self.__class__.__name__+str(self.dimensions)
        ...
        if self.dimensions == 2:
            ...
            ls_surface3d = LightSource(azdeg=0, altdeg=65)
            rgb = ls_surface3d.shade(Z, cmap)
            ...
            #plt.contourf(X,Y,Z,100,cmap=cmap, facecolors=rgb)

            ls = LightSource(azdeg=315, altdeg=45)
            plt.contourf(X,Y,ls.hillshade(Z, vert_exag=1),100,cmap='Greys')

            # for plotting optimization steps
            Xvals = [x[0] for x in xvals]
            Yvals = [x[1] for x in xvals]
            plt.plot(Xvals[0],Yvals[0],'o',ms=12, color='black', zorder=10)
            #plt.plot(Xvals[-1],Yvals[-1],'o',ms=15, color='red', zorder=10)
            
            plt.plot(xopt[0],xopt[1],'X',ms=18, color='black', zorder=10)
            #Xg = [x[0] for x in xglob]
            #Yg = [x[1] for x in xglob]
            #plt.plot(xglob[0],xglob[1],'*',ms=25, color='gold', zorder=5)
            
            plt.plot(Xvals, Yvals,marker='o', linestyle='-', color='tomato',lw=0.8)
            ...
            ax.plot(X, Y, 'k-', lw=1.5, zorder=30)
            xf, yf = self.global_optimum, self.fglob

            yopt = self.evaluator(atleast_1d(xopt[0]))

            yzero = self.evaluator(atleast_1d(xzero[0]))
            
            #ax.plot(xf, yf, 'r.', ms=11, zorder=40)
            #ax.scatter(xopt, yopt, edgecolors = 'g', facecolors='none',s=160, zorder=40, lw=2)
            #ax.scatter(xzero, yzero, edgecolors = 'black', facecolors='none',s=160, zorder=40, lw=2, linestyle='--')
        ...
        out_folder = os.path.join(os.getcwd(), 'figures')
        if not os.path.isdir(out_folder):
            os.makedirs(out_folder)
        
        filename = os.path.join(out_folder, '%s.png'%name)
        fig2.savefig(filename,dpi=600)
        #plt.close(fig)
        #del fig
        plt.show()
...
class Ackley(Benchmark):
    ...
    *Global optimum*: :math:`f(x_i) = 0` for :math:`x_i = 0` for :math:`i=1,...,n`      
    ...
    def evaluator(self, x_tuple, *args):
        ...
        a = 20.0; b = 0.5#0.2; 
        c = 2.0*pi
        #ansb =  -a*exp(-b*sqrt(1./self.dimensions*sum(x**2)))-exp(1./self.dimensions*sum(cos(c*x)))+a+exp(1.)
        ...
        ans= A + np.exp(1) + sum_sq_term + cos_term
        #print ('ans:', ans, 'ansb', ansb)
        return ans
...
class AE_func(Benchmark):
    """
    Simple parabola test objective function.

    This class defines the simple(st) optimization problem: A parabola. This 
    is a problem defined as follows:
    
    .. math::

        f_{\\text{AE_parabola}}(\\mathbf{x}) = \\sum_{i=1}^n x_i^2
        

    Here, :math:`n` represents the number of dimensions and :math:`x_i \\in [-1, 1]` for :math:`i=1,...,N`.

    .. figure:: figures/AE_parabola.png
        :alt: AE parabolic function
        :align: center
        
        **Two-dimensional parabolic function** 
    ...
    *Global optimum*: :math:`f(x_i) = 0.0` for :math:`x_i = 0` for :math:`i=1,...,N`  
    ...
    def __init__(self, dimensions=2):
        ...
        self.global_optimum = [0 for _ in range(self.dimensions)]
        self.fglob = 0.0*self.dimensions
...
class Damavandi(Benchmark):
    ...
    *Global optimum*: :math:`f(x_i) = 0.0` for :math:`x_i = 2` for :math:`i=1,...,n`  
    ...
    def __init__(self, dimensions=2):
        Benchmark.__init__(self, dimensions)
        # Commented by Azam as it is not possible to change dimens for this problem.
       # self.change_dimensionality = True 
...
class Rosenbrock(Benchmark):
    ...
    *Global optimum*: :math:`f(x_i) = 0` for :math:`x_i = 1` for :math:`i=1,...,n`  
    ...
    def __init__(self, dimensions=2):
        ...
        self.bounds = zip([-2.048] * self.dimensions, [2.048] * self.dimensions)
        self.custom_bounds = [(-2, 2), (-2, 2)]
